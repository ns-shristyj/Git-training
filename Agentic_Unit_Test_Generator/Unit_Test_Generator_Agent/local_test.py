...
Usage:
    # fast path — reads the local file, skips GitHub entirely
    python local_test.py <source_file>

    # exercises the real invocation contract: agent fetches the file from
    # GitHub at the given ref, using the PAT in Secrets Manager, exactly as
    # the workflow will
    python local_test.py <source_file> --via-github --ref <sha_or_branch> [--repo OWNER/NAME]
"""
import argparse
import json
import sys
...
def main():
    ...
    payload = {"file_path": args.source_file}
    if args.language:
    ...
    if result.get("status") == "ok":
        ...
        print("\nFAILED — no test code produced.", file=sys.stderr)
        sys.exit(1)
