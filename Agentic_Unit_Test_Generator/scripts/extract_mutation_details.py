...
Reads from .mutmut-cache to extract survived mutation details with rich descriptions.
"""
import json
import sys
import os
        ...
        if "mutations" in cache_data:
            for mut_id, mut_data in cache_data["mutations"].items():
                if mut_data.get("status") == "survived":
                    line_num = mut_data.get("line", "?")
                    operator = mut_data.get("mutator", mut_data.get("operator", "unknown"))
                    original = str(mut_data.get("original", "")).strip()
                    mutated = str(mut_data.get("mutated", "")).strip()
