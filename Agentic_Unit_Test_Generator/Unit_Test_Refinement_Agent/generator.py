"""Core refinement logic for the Unit Test Refinement Agent.
...
def refine_tests(
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            refined_test_code = extract_code_block(model_text)
        except ValueError:
        ...
        issues = _check_quality(refined_test_code, language)
        if not issues:
            return {
                "status": "ok",
