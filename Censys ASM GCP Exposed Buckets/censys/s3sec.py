...
def process(url, protocol="https"):
    ...
    if(b.text.find("<Error><Code>")>=0):
        ...
        print(f"[error: {code}] {url}.s3.amazonaws.com")
        list.append(f"[error: {code}] {url}.s3.amazonaws.com")
        if(code == "AccessDenied"):
            try:
                ...
                print(f"[read (--no-sign-request)] {url}.s3.amazonaws.com")
                list.append(f"[read (--no-sign-request)] {url}.s3.amazonaws.com")
            except:
                return
        return
        ...
        if(protocol=="http"):
            print(f"[error: ConnectionError({str(b.status_code)})] {url}.s3.amazonaws.com")
            list.append(f"[error: ConnectionError({str(b.status_code)})] {url}.s3.amazonaws.com")
        else:
    ...
    try:
        ...
        print(f"[write] {url}.s3.amazonaws.com")
        list.append(f"[write] {url}.s3.amazonaws.com")
        ...
        print(f"[delete] {url}.s3.amazonaws.com")
        list.append(f"[delete] {url}.s3.amazonaws.com")
