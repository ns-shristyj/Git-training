    ...
    def check_permissions(self, bucket_name):
        ...
        if self.client:
            ...
            if authenticated_permissions:
                authPermissionArray = list()
                if 'storage.buckets.setIamPolicy' in authenticated_permissions:
                    authPermissionArray.append("VULNERABLE TO PRIVILEGE ESCALATION")
                if 'storage.buckets.delete' in authenticated_permissions:
                    authPermissionArray.append('DELETEABLE')

                if 'storage.objects.list' in authenticated_permissions:
                    authPermissionArray.append('LISTABLE')
               
                if 'storage.objects.get' in authenticated_permissions:
                    authPermissionArray.append('READABLE')
                ...
                if 'storage.objects.create' in authenticated_permissions or 'storage.objects.delete' in authenticated_permissions or 'storage.objects.update' in authenticated_permissions:
                    authPermissionArray.append('WRITEABLE')
                   
                
                newAuthArray = map(str, authPermissionArray)
                authResult = ', '.join(newAuthArray)
                self.print(bucket_name + ' is: ' + authResult)

                # TO PRINT ALL THE FUNCTIONS ALLOWED
                # self.print('\n    All GCP Bucket Permissions Allowed:')
                # self.print(textwrap.indent('{}\n'.format(json.dumps(authenticated_permissions, indent=2)), '    '))
        ...
        if unauthenticated_permissions.get('permissions'):
            unauthPermissionArray = list()
            if 'storage.buckets.setIamPolicy' in unauthenticated_permissions['permissions']:
                unauthPermissionArray.append('VULNERABLE TO PRIVELIGE ESCALATION')
            if 'storage.buckets.delete' in unauthenticated_permissions['permissions']:
                unauthPermissionArray.append("DELETEABLE")
                
            if 'storage.objects.list' in unauthenticated_permissions['permissions']:
                unauthPermissionArray.append('LISTABLE')
                
            if 'storage.objects.get' in unauthenticated_permissions['permissions']:
                unauthPermissionArray.append("READABLE")
            ...
            if 'storage.objects.create' in unauthenticated_permissions['permissions'] or 'storage.objects.delete' in unauthenticated_permissions['permissions'] or 'storage.objects.update' in unauthenticated_permissions['permissions']:
                unauthPermissionArray.append("WRITEABLE")
                

            newUnauthArray = map(str, unauthPermissionArray)
            unauthResult = ', '.join(newUnauthArray)
            self.print(bucket_name + ' is: ' + unauthResult)

            # TO PRINT ALL THE FUNCTIONS ALLOWED
            # self.print('\n    All GCP Bucket Permissions Allowed:')
            # self.print(textwrap.indent('{}\n'.format(json.dumps(unauthenticated_permissions['permissions'], indent=2)), '    '))

        if not (authenticated_permissions or unauthenticated_permissions.get('permissions')):
            self.print(bucket_name + ': exists but has no allowed permissions')
...
if __name__ == '__main__':
    ...
    group.add_argument('--check-list', required=False, default="gcpUrls.txt", help='Check a list of buckets in the given file, one per line.')
    ...
    parser.add_argument('-u', '--unauthenticated', required=False, default=True, action='store_true', help='Force an unauthenticated scan (you will not be prompted for credentials)')
    parser.add_argument('-o', '--out-file', required=False, default="gcpSecUrls.txt", help='The path to a log file to write the scan results to. The file will be created if it does not exist and will append to it if it already exists. By default output will only print to the screen.')
