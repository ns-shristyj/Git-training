...
def test_fetch_live_auth0_actions_success_no_drift(monkeypatch):
    ...
    live_actions_payload = [
        ...
        },
        {
            ...
            "status": "built",
            "updated_at": "2026-08-01T00:00:00.000Z",
            "code": "if (!entitlements.includes(x) && !birthright.includes(x) && !metadata.block) deny();",
        },
    ]
...
def test_expected_code_markers_are_verified_against_real_archived_source():
    ...
    action_to_filename = {
        # Birthright computation & enforcement
        "NetskopeID-Sync-1": "auth0-action-netskopeid-sync-1.md",
        "NetskopeID-Sync-2": "auth0-action-netskopeid-sync-2.md",
        "Gatekeeper": "auth0-action-gatekeeper.md",
        "RBAC - Consolidated": "auth0-action-rbac-consolidated.md",
        # Onboarding & verification
        "Email Verification v2": "auth0-action-email-verification-v2.md",
        "Registration-Forms": "auth0-action-registration-forms.md",
        "Account Migration Acknowledgement": "auth0-action-account-migration-acknowledgement.md",
        "Privacy Policy Acknowledgement": "auth0-action-privacy-policy-acknowledgement.md",
        "MFA - Consolidated": "auth0-action-mfa-consolidated.md",
    }
...
def test_entrypoint_always_attempts_live_fetch_for_comprehensive_monitoring(monkeypatch):
    """Agent 4 now ALWAYS fetches all 9 Auth0 Actions (per ISI-Solutions-Architecture)
    for comprehensive monitoring of the entire auth/provisioning flow, not just
    birthright computation. This gives L1 support visibility into Email Verification,
    Registration Forms, Consent, and MFA Actions alongside birthright/enforcement,
    regardless of whether there's a gap to diagnose. Non-fatal if fetch fails --
    falls back to offline snapshot."""
    calls = []
    monkeypatch.setattr(
    ...
    assert result["birthright_evaluation"]["match"] is True
    # fetch_live_auth0_actions SHOULD have been called (always, for monitoring)
    assert len(calls) == 1
    # All 9 actions fetched per ISI-Solutions-Architecture (CIAM Phase 2)
    assert "NetskopeID-Sync-1" in calls[0]
    assert "Email Verification v2" in calls[0]
...
# --- GROUP_BASED_CONNECTION ("NSKP-Preview" / internal-user) coverage ---
#
# Internal/federated users authenticate via the "NSKP-Preview" Auth0
# connection and get their birthright from Okta group membership
# (NetskopeID-Sync-1), not from Salesforce Account Status (NetskopeID-
# Sync-2) -- verified live against the tenant on 2026-08-14, including in
# Sync-2's own source (`birthright_eligible = event.connection.name !==
# "NSKP-Preview"`). Before this fix, such a user had no Salesforce account
# at all (account_status=None) and was always misdiagnosed as
# "account_status_unknown", regardless of what portal group they actually
# belonged to.

def test_derive_group_based_persona_support_group():
    persona, expected = derive_group_based_persona(["prod-cust-nsid-supportportal-user-grn"])
    assert persona == "Employee"
    assert set(expected) == {"Support", "Academy", "Notification", "Dashboard"}


def test_derive_group_based_persona_partner_and_prime_union():
    persona, expected = derive_group_based_persona([
        "prod-cust-nsid-partnerportal-user-grn",
        "prod-cust-nsid-partnerportal-prime-user-grn",
    ])
    assert persona == "Employee"
    assert set(expected) == {"Prime", "Partner", "Academy", "Notification", "Dashboard"}


def test_derive_group_based_persona_no_recognized_group():
    persona, expected = derive_group_based_persona([])
    assert persona == "Employee (No Portal Group)"
    assert expected == []

    persona2, expected2 = derive_group_based_persona(["some-unrelated-group"])
    assert persona2 == "Employee (No Portal Group)"
    assert expected2 == []


def test_evaluate_birthright_group_based_ignores_salesforce_fields():
    """A GROUP_BASED_CONNECTION user with NO Salesforce account at all
    (account_status=None) must still get a real persona/expected_birthright
    from their Okta groups, not fall through to the "Individual" Salesforce
    default."""
    evaluation = evaluate_birthright(
        ...
        actual_birthright=[],
        entitlements=[],
        connection=GROUP_BASED_CONNECTION,
        groups=["prod-cust-nsid-supportportal-user-grn"],
    )
    assert evaluation.persona == "Employee"
    assert evaluation.persona_source == "okta_groups"
    assert set(evaluation.expected_birthright) == {"Support", "Academy", "Notification", "Dashboard"}
    assert set(evaluation.missing_keywords) == {"Support", "Academy", "Notification", "Dashboard"}


def test_evaluate_birthright_non_group_connection_unchanged():
    """Default path (connection=None, the only thing today's orchestrator
    sends) must be bit-for-bit identical to before this fix."""
    evaluation = evaluate_birthright(
        account_status="Customer",
        active_tenant_count=1,
        actual_birthright=[],
    ...
    assert evaluation.persona == "Customer"
    assert evaluation.persona_source == "salesforce_account_status"


def test_classify_fix_complexity_group_based_no_recognized_group_escalates():
    fc = classify_fix_complexity(
        missing_keywords=[],
        extra_keywords=[],
        account_status=None,
        ...
        birthright_correct_but_access_denied=False,
        is_group_based=True,
        persona="Employee (No Portal Group)",
    )
    assert fc.complexity == "ESCALATE_TO_L2"
    assert "Okta" in fc.reason or "okta" in fc.reason.lower()


def test_classify_fix_complexity_group_based_does_not_hit_account_status_unknown():
    """The account_status_unknown branch must never fire for a group-based
    user just because they (legitimately) have no Salesforce account --
    that used to be the exact misdiagnosis this fix corrects."""
    fc = classify_fix_complexity(
        missing_keywords=["Support"],
        extra_keywords=[],
        account_status=None,
    ...
    assert fc.reason != "account_status_unknown -- cannot safely determine correct entitlement without confirmed account data."
    assert fc.complexity == "SIMPLE_FIX"
    assert "NetskopeID-Sync-1" in fc.reason
    assert "Okta group membership" in fc.reason


def test_identify_failing_workflow_group_based_names_sync_1():
    with patch("agent.requests.get") as mock_get, patch("agent.requests.post") as mock_post:
        mock_post.return_value = mock_response({"access_token": "tok", "expires_in": 3600})
        mock_get.return_value = mock_response({"actions": [
            {"name": "NetskopeID-Sync-1", "id": "548246b5-264d-4b2f-a91d-2d766cee1933",
             "status": "built", "code": "event.user.groups NSKP-Preview prod-cust-nsid birthright"},
            {"name": "Gatekeeper", "id": "35d76097-24c1-4b5b-b3cc-e853e286b7e6",
             "status": "built", "code": "birthright entitlements"},
        ]})
        live_actions, error = fetch_live_auth0_actions(["NetskopeID-Sync-1", "Gatekeeper", "RBAC - Consolidated"])

    wf = identify_failing_workflow("Support", ["Support"], live_actions, error, is_group_based=True)

    assert wf.workflow_name == "NetskopeID-Sync-1"
    assert "Okta" in wf.note
    assert "Salesforce" not in wf.note
    assert wf.workflow_script_ref == "548246b5-264d-4b2f-a91d-2d766cee1933"


def test_identify_failing_workflow_default_still_names_sync_2():
    """is_group_based defaults to False -- existing call sites (no keyword
    arg passed) must keep naming NetskopeID-Sync-2, unchanged."""
    wf = identify_failing_workflow("Support", ["Support"])
    assert wf.workflow_name == "NetskopeID-Sync-2"
    assert "Salesforce" in wf.note


def test_entrypoint_group_based_connection_end_to_end():
    """Full evaluate_ciam_case() flow for an internal user: no Salesforce
    account, connection=NSKP-Preview, in the support portal Okta group but
    missing all four resulting keywords -- must classify as SIMPLE_FIX
    against Okta groups, not escalate as account_status_unknown."""
    with patch("agent.fetch_live_auth0_actions", lambda names: ({}, None)):
        with patch("agent.requests.post") as mock_post:
            mock_post.return_value = mock_response(mock_empty_kb_response())
            result = evaluate_ciam_case({
                "account_status": None,
    ...
    be = result["birthright_evaluation"]
    assert be["persona"] == "Employee"
    assert be["persona_source"] == "okta_groups"
    assert set(be["expected_birthright"]) == {"Support", "Academy", "Notification", "Dashboard"}
    assert result["fix_classification"]["complexity"] == "SIMPLE_FIX"
    assert result["fix_classification"]["reason"] != "account_status_unknown -- cannot safely determine correct entitlement without confirmed account data."
    assert result["workflow_identification"]["workflow_name"] == "NetskopeID-Sync-1"
