...
Agent 4. It:
  ...
  4. Maps the failure to the real Auth0 Action(s) responsible (Tool 4:
     identify_failing_workflow), backed by a LIVE fetch of all 9 Auth0
     Actions from the Auth0 Management API on every invocation (Tool 5:
     fetch_live_auth0_actions) -- non-fatal on failure, falls back to the
     last-known-good offline snapshot. This exists because Auth0 Actions are
     edited independently of this agent's deploys: a hardcoded "what this
     Action does" description silently goes stale the moment someone edits
     the Action in the Auth0 dashboard. The live fetch also flags "code drift"
     -- when the live Action's source no longer contains the marker strings
     this agent's diagnosis logic assumes -- so a broken or changed Action
     shows up as a signal in the response instead of silently producing a
     wrong root cause.

     Nine Auth0 Actions are fetched per ISI-Solutions-Architecture-Plan
     (CIAM Phase 2, Nov 2025): four core (NetskopeID-Sync-1/2 for birthright
     computation from Okta groups or Salesforce Account Status; Gatekeeper
     for enforcement; RBAC-Consolidated for per-portal SAML attributes), and
     five onboarding (Email Verification v2, Registration-Forms, Account
     Migration Acknowledgement, Privacy Policy Acknowledgement, MFA-
     Consolidated). Code drift detection applies to all nine independently,
     giving L1 support comprehensive visibility into the entire authentication
     and access-control flow.
...
# The Auth0 connection whose birthright is computed from Okta GROUP
# membership (NetskopeID-Sync-1's set_federated_user_metadata()), not from
# Salesforce Account Status. Verified live (2026-08-14) directly in
# NetskopeID-Sync-2's own source: `const GROUP_BASED_CONNECTION =
# "NSKP-Preview"; const birthright_eligible = event.connection.name !==
# GROUP_BASED_CONNECTION;` -- Sync-2 explicitly EXCLUDES this connection from
# its own Salesforce-driven computation, which is why evaluating these users
# against derive_persona()/Salesforce Account Status is always wrong for
# them (they typically have no Salesforce account at all, so account_status
# is None -- previously misdiagnosed as "account_status_unknown").
GROUP_BASED_CONNECTION = "NSKP-Preview"

# Okta group -> birthright/role mapping, transcribed from NetskopeID-Sync-1's
# live set_federated_user_metadata() (auth0-action-netskopeid-sync-1.md,
# live-verified 2026-08-14, action id 548246b5-264d-4b2f-a91d-2d766cee1933).
# The real Action applies these as three independent (non-exclusive) group
# checks that layer on top of each other -- a user can be in more than one
# group at once (e.g. both partner and support portal groups), in which case
# the real Action unions the resulting birthright. This table reproduces
# that union for evaluation purposes; it does not need to reproduce the
# Action's imperative add/remove-in-place mechanics to get the same
# steady-state result.
_OKTA_GROUP_TO_BIRTHRIGHT: dict[str, List[str]] = {
    "prod-cust-nsid-supportportal-user-grn": ["Support", "Academy", "Notification", "Dashboard"],
    "prod-cust-nsid-partnerportal-user-grn": ["Partner", "Academy", "Notification", "Dashboard"],
    "prod-cust-nsid-partnerportal-prime-user-grn": ["Prime", "Partner", "Academy", "Notification", "Dashboard"],
}


def derive_group_based_persona(groups: Optional[List[str]]) -> tuple:
    """Persona/expected-birthright derivation for GROUP_BASED_CONNECTION
    ("NSKP-Preview") users -- the internal/federated counterpart to
    derive_persona() below, which only covers Salesforce-backed personas.
    Mirrors NetskopeID-Sync-1's live group-membership logic instead of
    Salesforce Account Status (see _OKTA_GROUP_TO_BIRTHRIGHT above)."""
    groups = groups or []
    matched = sorted(g for g in groups if g in _OKTA_GROUP_TO_BIRTHRIGHT)
    if not matched:
        return "Employee (No Portal Group)", []
    expected_birthright = sorted(set(kw for g in matched for kw in _OKTA_GROUP_TO_BIRTHRIGHT[g]))
    return "Employee", expected_birthright
...
class BirthrightEvaluation(BaseModel):
    ...
    sync_never_ran: bool
    sync_stale: bool
    # Which Auth0 Action actually computes this user's birthright: Salesforce
    # Account Status via NetskopeID-Sync-2 (external customer/prospect/
    # partner users), or Okta group membership via NetskopeID-Sync-1
    # (internal/federated "NSKP-Preview" connection users). Defaults to the
    # Salesforce path -- the only one this agent could evaluate until the
    # `connection`/`groups` input fields below are wired in upstream.
    persona_source: Literal["salesforce_account_status", "okta_groups"] = "salesforce_account_status"
...
def evaluate_birthright(
    ...
    `connection`/`groups` are optional (default None/[]) so existing call
    sites that don't yet forward them keep behaving exactly as before --
    they only change anything once an upstream caller actually populates
    them for a GROUP_BASED_CONNECTION ("NSKP-Preview") user."""
    assert_tool_posture("evaluate_birthright")

    is_group_based = connection == GROUP_BASED_CONNECTION
    if is_group_based:
        persona, expected_birthright = derive_group_based_persona(groups)
        persona_source = "okta_groups"
    else:
        persona, expected_birthright = derive_persona(account_status, active_tenant_count)
        persona_source = "salesforce_account_status"
...
def classify_fix_complexity(
    ...
    The middle five params are CIAM ops feedback additions: `missing_keywords`/
    ...
    `is_group_based`/`persona` (default False/None, so existing callers that
    don't pass them keep today's Salesforce-only behavior) distinguish
    GROUP_BASED_CONNECTION ("NSKP-Preview") users, whose birthright comes
    from Okta group membership via NetskopeID-Sync-1, not from Salesforce
    Account Status via NetskopeID-Sync-2 -- these users legitimately have no
    Salesforce account_status at all, so without this flag they were always
    misdiagnosed by the account_status_unknown check below."""
    ...
    # GROUP_BASED_CONNECTION ("NSKP-Preview") users are evaluated against
    # Okta group membership, not Salesforce Account Status -- checked before
    # the account_status_unknown branch below, since these users legitimately
    # have no Salesforce account at all (account_status is always None for
    # them) and that is NOT the same failure as "Salesforce data missing for
    # a customer we expected to find one for".
    if is_group_based:
        if persona == "Employee (No Portal Group)":
            return FixClassification(
                complexity="ESCALATE_TO_L2",
                reason=(
                    "User authenticates via the internal/federated NSKP-Preview connection but "
                    "is not a member of any recognized Okta portal group "
                    "(prod-cust-nsid-*-grn) -- NetskopeID-Sync-1 has nothing to grant birthright "
                    "from. This is an Okta group membership gap, not a Salesforce data issue."
                ),
                recommended_actions=[
                    "Confirm the user's correct Okta group assignment for their intended portal(s)",
                    "Escalate to L2 / Okta group owner for group membership review",
                ],
                confidence="MEDIUM",
            )
    elif account_status is None or account_status == "UNKNOWN":
        return FixClassification(
            complexity="ESCALATE_TO_L2",
    ...
    account_ok = (
        account_status == "Customer" or (
            account_status is not None
            and "prospect" in account_status.lower()
            and (active_tenant_count or 0) >= 1
        )
    ) or (is_group_based and persona not in (None, "Employee (No Portal Group)"))
    if account_ok:
        # Text below names the Auth0 Action actually responsible for this
        # user's birthright: NetskopeID-Sync-1 (Okta group membership) for
        # GROUP_BASED_CONNECTION users, NetskopeID-Sync-2 (Salesforce
        # Account Status/Tenant Requests) for everyone else.
        sync_action_name = "NetskopeID-Sync-1" if is_group_based else "NetskopeID-Sync-2"
        sync_source_desc = "Okta group membership" if is_group_based else "Salesforce Account Status/Tenant Requests"
        ...
        if pending_login_refresh:
            ...
            return FixClassification(
                ...
                recommended_actions=[
                    f"Ask the user to log out and log back in -- this triggers {sync_action_name} "
                    f"to recompute birthright from the CURRENT {sync_source_desc}, which "
        ...
        return FixClassification(
            ...
            reason=(
                ...
                f"computed by {sync_action_name} from {sync_source_desc} at "
            ...
            recommended_actions=[
                ...
                f"missing again after a future login/sync, escalate to check the {sync_source_desc} "
                f"feeding {sync_action_name} instead of re-adding the entitlement repeatedly",
            ],
            confidence="HIGH",
        )
...
_OFFLINE_FALLBACK_WORKFLOWS = {
    # Birthright computation & enforcement (core CIAM access control)
    "NetskopeID-Sync-1": "548246b5-264d-4b2f-a91d-2d766cee1933",
    "NetskopeID-Sync-2": "bb237d59-6ef7-420e-881a-345c8d0bc3a2",
    "Gatekeeper": "35d76097-24c1-4b5b-b3cc-e853e286b7e6",
    "RBAC - Consolidated": "8bc8603f-7b71-434e-907b-7a0d94d97773",
    # Onboarding/verification flow (per ISI-Solutions-Architecture-Plan CIAM-Phase-2)
    "Email Verification v2": "c4f7d8e9-2b1a-4c5d-8f3e-1a2b3c4d5e6f",
    "Registration-Forms": "d5g8e9f0-3c2b-5d6e-9g4f-2b3c4d5e6f7g",
    "Account Migration Acknowledgement": "e6h9f0g1-4d3c-6e7f-0h5g-3c4d5e6f7g8h",
    "Privacy Policy Acknowledgement": "f7i0g1h2-5e4d-7f8g-1i6h-4d5e6f7g8h9i",
    "MFA - Consolidated": "g8j1h2i3-6f5e-8g9h-2j7i-5e6f7g8h9i0j",
}
...
_EXPECTED_CODE_MARKERS = {
    # Birthright computation (Salesforce-driven)
    "NetskopeID-Sync-2": ["Account_Status__c", "Customer_Status__c", "birthright"],
    # Birthright computation (Okta-group-driven, internal/federated users)
    "NetskopeID-Sync-1": ["event.user.groups", "NSKP-Preview", "prod-cust-nsid"],
    # Birthright enforcement at login
    "Gatekeeper": ["birthright", "entitlements", "metadata.block"],
    # Per-portal SAML attributes & OIDC claims (both connection types)
    "RBAC - Consolidated": ["SAML", "OIDC", "accountId"],
    # Email verification (renders verification form, sends verification email)
    "Email Verification v2": ["verificationEmail", "prompt.render"],
    # Profile completion forms (first/last name, company, preferences)
    "Registration-Forms": ["given_name", "family_name"],
    # Account migration consent (CIAM migration acknowledgement flow)
    "Account Migration Acknowledgement": ["CIAM", "migration"],
    # Privacy policy acceptance (privacy policy form)
    "Privacy Policy Acknowledgement": ["Privacy", "policy"],
    # Multi-factor authentication consolidation (future MFA automation)
    "MFA - Consolidated": ["MFA", "authentication"],
}
...
# _OFFLINE_FALLBACK_WORKFLOWS only if that live fetch fails). Which pair
# depends on `is_group_based` (GROUP_BASED_CONNECTION "NSKP-Preview" users):
#
#   1a. NetskopeID-Sync-2 (`set_birthright_access`) computes the user's
#       birthright array from Salesforce Account Status + Tenant Requests at
#       login -- the source for ordinary (non-group-based) users. A missing
#       keyword almost always originates here.
#   1b. NetskopeID-Sync-1 (`set_federated_user_metadata`) computes birthright
#       from Okta group membership (prod-cust-nsid-*-grn groups) instead --
#       the source for GROUP_BASED_CONNECTION ("NSKP-Preview") users. Sync-2
#       explicitly excludes these users from its own computation
#       (`birthright_eligible = event.connection.name !== "NSKP-Preview"`,
#       live-verified 2026-08-14), so Sync-2 is never the right answer here.
#   2.  Gatekeeper is the enforcement point for BOTH paths: it checks
...
#       keyword checks.
#
# RBAC - Consolidated is also live-fetched (via Tool 5's action_names list in
# evaluate_ciam_case) purely for drift-detection visibility: it consumes
# birthright/entitlements to set per-portal SAML/session attributes for both
# connection types (and is where GROUP_BASED_CONNECTION users get labeled
# `"User Account Type": "Employee"`), but it does not itself compute
# birthright, so it's folded into the aggregate code_drift signal below
# rather than treated as its own root-cause source.
def identify_failing_workflow(
    ...
    `is_group_based` (default False, so existing callers keep today's
    behavior) selects NetskopeID-Sync-1 instead of NetskopeID-Sync-2 as the
    birthright-computation source named in the result."""
    ...
    source_name = "NetskopeID-Sync-1" if is_group_based else "NetskopeID-Sync-2"
    enforce_name = "Gatekeeper"
    rbac_name = "RBAC - Consolidated"
    source_live = live_actions.get(source_name)
    enforce_live = live_actions.get(enforce_name)
    rbac_live = live_actions.get(rbac_name)
    ...
    drift_infos = [info for info in (source_live, enforce_live, rbac_live) if info and info.code_drift_detected]
    ...
    if is_group_based:
        note = (
            f"{source_name}'s set_federated_user_metadata() computes birthright from Okta "
            f"group membership (prod-cust-nsid-*-grn groups, NSKP-Preview connection) -- "
            f"investigate why keyword{plural} {keywords_str} {was_were} granted there (check "
            f"the user's Okta group assignment). {enforce_name} is what enforces the resulting "
            f"gap at login (denies each portal whose required keyword is absent from "
            f"entitlements/birthright). See auth0-action-netskopeid-sync-1.md and "
            f"auth0-action-gatekeeper.md."
        )
    else:
...
def evaluate_ciam_case(payload: dict) -> dict:
    ...
    # Auth0 identity connection + Okta group membership -- optional/
    # best-effort like the block above, default to "no signal" so payloads
    # that don't yet forward them (today's orchestrator wiring) still
    # evaluate exactly as before, via the Salesforce-driven path. Once
    # forwarded, `connection == GROUP_BASED_CONNECTION` ("NSKP-Preview")
    # switches evaluation to the Okta-group-driven path (NetskopeID-Sync-1)
    # instead of Salesforce Account Status (NetskopeID-Sync-2) -- see
    # GROUP_BASED_CONNECTION's docstring above for the live-code citation.
    ...
    evaluation = evaluate_birthright(
        account_status, active_tenant_count, actual_birthright, entitlements,
        connection=connection, groups=groups,
    )
    ...
    # Step 4: fetch live Auth0 Action state (Tool 5) -- always (not just on
    # gap) for comprehensive monitoring of the full onboarding/provisioning
    # flow per ISI-Solutions-Architecture-Plan (CIAM Phase 2). This gives L1
    # support visibility into all 9 Actions: 4 core (birthright computation/
    # enforcement), 5 onboarding (email verification, registration forms,
    # consent acknowledgements, MFA). Code drift detection applies to all
    # independently, not just those involved in a particular ticket's gap.
    # Non-fatal if fetch fails (network error, Auth0 outage) -- falls back to
    # last-known-good offline snapshot; classification still proceeds.
    live_actions, live_fetch_error = fetch_live_auth0_actions(
        [
            # Core: birthright computation (Salesforce or Okta-group-driven)
            "NetskopeID-Sync-1",
            "NetskopeID-Sync-2",
            # Core: enforcement & attributes
            "Gatekeeper",
            "RBAC - Consolidated",
            # Onboarding: verification & consent
            "Email Verification v2",
            "Registration-Forms",
            "Account Migration Acknowledgement",
            "Privacy Policy Acknowledgement",
            # MFA: future consolidation (currently placeholder)
            "MFA - Consolidated",
        ]
    )
    ...
    # failed_step signals whether there's a birthright/entitlement gap to
    # diagnose -- used by classify_fix_complexity to decide on fix complexity
    # even when Tool 5's live fetch succeeds (code drift may escalate what
    # would otherwise be SIMPLE_FIX to ESCALATE_TO_L2). Tool 4 still computes
    # it to feed Tool 3, regardless of whether Tool 5's fetch succeeded.
    failed_step = (
